# 申请创建 connectors-oceanbase 项目

## 项目基本信息

项目名称：connectors-oceanbase

项目目的：基于es的connectors开源项目改造而来，主要是提供一个轻量级的数据迁移框架，复用现有的es的连接器，可以将数据迁移到oceanbase/seekdb。

项目责任人(github id)：
- PMC: sc-source
- PMC：xxsc0529

开源协议：Elastic License 2.0

> 建议采用 Apache 2.0，如果不采用 Apache 2.0 协议，请注明原因

本仓库的 OceanBase 后端实现是基于 https://github.com/elastic/connectors 上游项目改造而来。上游代码使用的是 Elastic License 2.0。由于开源协议是由版权/权利人授予的，
我们不能在未获得上游权利人额外授权的情况下，单方面将包含上游受版权保护代码的部分重新声明为 Apache 2.0。


## 开源项目检查清单

> 检查清单是为了让项目更规范，让社区用户更容易使用。应该在项目创建后，尽快补充清单中的信息。

- [Y] 包含 README.md
- [N] 工程类项目需要包含 CONTRIBUTING.md。参考 OceanBase 社区 [CONTRIBUTING 文件](https://github.com/oceanbase/.github/blob/main/CONTRIBUTING.md)
- [N] 包含文件 CODE_OF_CONDUCT.md （没有此文件将使用社区现有的[行为准则文件](https://github.com/oceanbase/.github/blob/main/CODE_OF_CONDUCT.md) ）
- [x] 工程类项目包含用户安装指导说明（通常在README.md中说明）
- [x] 工程类项目包含用户使用指导说明（通常在README.md中说明）
- [x] 工程类项目代码类项目包含编译指导说明（通常在README.md中说明）

## 投票截止时间

如果不满足投票条件，此投票将在 2026 年 04 月 8 日截止。

> 满足投票条件是说投票已经成功，比如已经有不少于 2/3 的 TOC 投票，或者投票失败，比如有一半的 TOC 反对。

## 投票结果

参考 [创建 connectors-oceanbase 项目投票结果](https://github.com/oceanbase/community/pull/71)。
