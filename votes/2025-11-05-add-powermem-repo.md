...
项目目的：为AI开发者提供智能记忆管理服务，为LLM应用提供持久化记忆层。支持多智能体协作、智能记忆管理（基于艾宾浩斯遗忘曲线）、自动重要性评分、多种存储后端（OceanBase、SQLite、PostgreSQL）以及丰富的LLM和嵌入模型集成。
...
如果不满足投票条件，此投票将在 2025 年 11 月 15 日截止。

> 满足投票条件是说投票已经成功，比如已经有不少于 2/3 的 TOC 投票，或者投票失败，比如有一半的 TOC 反对。

## 投票结果

参考 [创建 powermem 项目投票结果](https://github.com/oceanbase/community/pull/49)。
